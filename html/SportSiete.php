...
*/
   ...
   public static function grabarapuesta($sesion,$monto,$datos)
	{
        global $dbconn;
...
/*
        ...
        $result = pg_execute($dbconn, "consultarDisponible", array($sesion));
...
//   	foreach($result as $dato)
        foreach($datos_d as $dato)
	{
        ...
        $result = pg_execute($dbconn, "descontarDisponible", array($sesion,$monto));

        if (!$result)
             return json_encode(array("Error" => "-1", "ErrorDescripcion" => "Error descontando el monto"));
        $result = pg_prepare($dbconn, "grabarapuesta",
                                      "insert into apuesta(codigo,usuario,monto,factor_pagado,instante_apuesta,estado,verificacion,doublechance) select nextval('conse_apuesta'), s.usuario,$2,0,now(),-10,substr(md5(now()::text || random()::text),1,4)," . ($DoubleChance?"true":"false") .
                                                          " from sesion s where s.id=$1".
                                      " returning codigo");
        $result = pg_execute($dbconn, "grabarapuesta", array($sesion,$monto));
/*
        $result = pg_prepare($dbconn, "grabarapuesta",
...
*/
   	...
   	foreach($datos_d as $dato)
	{
        ...
        $result = pg_prepare($dbconn, "grabarAutoAprobacion",
					...
					.	" RETURNING estado");
        $result = pg_execute($dbconn, "grabarAutoAprobacion", array($sesion,$codigo));
   ...
   public static function ApuestaAnular($sesion,$Apuesta)
	...
	error_log("Anulada Apuesta " . $Apuesta . " por id " . $sesion);
     ...
     public static function UsuarioRegistrar($Login,$Clave)
     {
        global $dbconn;
...
//				.	" LEFT OUTER JOIN (SELECT MAX(codigo) as ncod FROM usuario u) AS t2 ON 1=1"
        ...
        $result = pg_execute($dbconn, "crear_usuario", array($MinNombreUsuario,$ClavePulida));
...
//error_log("u : " . $MinNombreUsuario . " c : " . $Clave);
  ...
  public static function IngresarUsuario($login,$Clave)
     {
        global $dbconn;

        $NombreUsuario = strtolower($login);


        $result = pg_prepare($dbconn, "crear_usuario",
              " INSERT INTO usuario" .
              " SELECT COALESCE(ncod,0) + 1" .
                    " ,$1::text" .
                    " ,$1::text || $2" .
              " FROM   (select 1) AS t" .
                           " LEFT OUTER JOIN (SELECT MAX(codigo) as ncod FROM usuario u) AS t2 ON 1=1" .
        ...
        $result = pg_execute($dbconn, "crear_usuario", array($NombreUsuario,$Clave));
        ...
        $result = pg_execute($dbconn, "encriptar_clave", array($NombreUsuario));

        if (!$result)
          return json_encode(array("Error" => "-1", "ErrorDescripcion" => "No se pudo activar la clave"));


        return json_encode(array("Retorno" => "1"));
     }
